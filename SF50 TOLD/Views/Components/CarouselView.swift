  ...
  var body: some View {
    ...
    ScrollView(.horizontal) {
      LazyHStack(alignment: .top, spacing: 0) {
        ForEach(Array(dataCollection.enumerated()), id: \.offset) { index, item in
          content(item)
    ...
    .onChange(of: scrollPositionId) { _, newId in
      if let newId, let index = dataCollection.firstIndex(where: { $0[keyPath: idKeyPath] == newId }) {
    ...
    .onChange(of: currentIndex) { oldIndex, newIndex in
      ...
      let count = dataCollection.distance(from: dataCollection.startIndex, to: dataCollection.endIndex)
